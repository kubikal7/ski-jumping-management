...
export default function UsersPage() {
  ...
  const createUser = async () => {
    try {
      await api.post('/users', creatingUser);
      setDrawerOpen(false);
      fetchUsers();
    } catch (e) {
      showToast('Błąd tworzenia użytkownika', 'error');
    }
  };
    ...
    <div>
      <div className='tool-title'>
        ...
        {(roles.includes("ADMIN") || roles.includes("MANAGER")) &&
          <button className="btn primary" onClick={() => { setCreatingUser(emptyUserRequest()); setDrawerOpen(true); }}>
            + Nowy użytkownik
          </button>
        }
      ...
      <div className="column">
        <div className="card gap" style={{ alignItems: 'center' }}>
          ...
          <input
            ...
            style={{ width: 160 }}
          />
          <RoleSelector selectedRoleIds={filters.roleIds} onChange={roleIds => setFilters(f => ({ ...f, roleIds: roleIds }))} isAdmin={true}/>
        ...
        <Drawer open={drawerOpen} title="Nowy użytkownik" onClose={() => setDrawerOpen(false)}>
          <UserForm
            ...
            onSubmit={async (userData) => {
              try {
                ...
                if (e.response?.status === 409) {
                  ...
                  setServerError('Błąd tworzenia użytkownika');
                }
              }
            }}
          />
